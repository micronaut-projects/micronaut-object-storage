...
It also implements api:objectstorage.multipart.MultipartObjectStorageOperations[] for multipart upload lifecycle support.
...
snippet::example.oraclecloud.ObjectStorageClientBuilderCustomizer[project-base="doc-examples/example", source="main", indent="0", tags="class"]

See api:objectstorage.multipart.MultipartObjectStorageOperations[] and the <<multipartUploads,Multipart Uploads>> guide for the
