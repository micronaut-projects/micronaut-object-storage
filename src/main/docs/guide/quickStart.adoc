...
<1> To proactively revoke a previously generated URL, use `invalidatePresignedRequest(URI)` of api:objectstorage.ObjectStorageOperations[]. Only a few providers support explicit invalidation (e.g Oracle); in those cases, an `UnsupportedOperationException` will be thrown and you should rely on expiration instead.
