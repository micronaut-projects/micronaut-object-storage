...
snippet::example.LogoService[project-base="doc-examples/example", source="main", indent="0", tags="class"]

<1> The api:io.micronaut.core.io.ResourceResolver[] bean aggregates the `ResourceLoader` beans published by the
    configured storages.
<2> The `pictures` storage resolves the URI to the object with the key `avatars/logo.png`.
