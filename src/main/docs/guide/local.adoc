...
temp directory. Rollback snapshots are removed on normal completion and are not part of the one-day replacement temp
...
bucket in the same JVM. These guarantees are per file. Object byte replacement and metadata persistence are separate
operations; the local provider does not provide a single crash-atomic transaction across both. Custom metadata providers,
including database-backed implementations, are responsible for their own metadata durability and any higher-level
